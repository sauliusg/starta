...
int uarrays_trace_on( int trace_flag )
{
    int old_trace_flag = uarrays_trace;
    uarrays_trace = trace_flag;
...
/*
 * ARRAY_UI2LL convert unsigned integer array to a 'long long' array.
 * 
 * bytecode:
 * ARRAY_UI2LL
 * 
 * stack:
 * llong array, uint array -> llong array
 * 
 */
...
int ARRAY_UI2LL( INSTRUCTION_FN_ARGS )
{
...
#ifdef packed_type
    unsigned int *src_array = STACKCELL_PTR( istate.ep[0] );
    long long *dst_array = STACKCELL_PTR( istate.ep[1] );
#elif full_stackcell
    stackcell_t *src_array = STACKCELL_PTR( istate.ep[0] );
    stackcell_t *dst_array = STACKCELL_PTR( istate.ep[1] );
#elif split_stackcell
    ...
    if( src_array && dst_array ) {
        ...
        for( i = 0; i < length; i++ ) {
            ...
            dst_array[i] = src_array[i];
#elif full_stackcell
            dst_array[i].num.ll = src_array[i].num.ui;
#elif split_stackcell
            dst_array[i].ll = src_array[i].ui;
#endif
        }
    }
