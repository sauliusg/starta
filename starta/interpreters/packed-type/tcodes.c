 ...
 *
 * bytecode:
 * MKLIST size nref next_link_offset value_offset value_size
 *
 * stack:
...
int MKLIST( INSTRUCTION_FN_ARGS )
{
    ...
    ssize_t value_size = istate.code[istate.ip+5].ssizeval;
    ssize_t i;
    ...
    for( i = 1; i <= nexpressions; i ++ ) {
        ...
        if( value_offset >= 0 ) {
            ...
            if( copy_size > sizeof(istate.ep[i].num) )
                copy_size = sizeof(istate.ep[i].num);
...
int MKARRAY( INSTRUCTION_FN_ARGS )
{
    ...
    ptr = bcalloc_array( size, nele, nref, EXCEPTION );
...
int PMKARRAY( INSTRUCTION_FN_ARGS )
{
    ...
    ptr = bcalloc_array( sizeof(void*), nele, 1, EXCEPTION );
...
int CLONE( INSTRUCTION_FN_ARGS )
{
    ...
    if( nele >= 0 ) {
        ...
        ptr = bcalloc( size, element_size, -1, nref, EXCEPTION );
    }
#else
    /* printf( ">>> size = %d, esize = %d, nele = %d, nref = %d\n",
       size, element_size, nele, nref ); */
    ptr = bcalloc( size, element_size, nele, nref, EXCEPTION );
#endif
...
static int deep_clone( alloccell_t *ptr, int level )
{
    ...
    for( i = 0; abs(i) < abs(length); i += delta ) {
        ...
        if( element ) {
            ...
            ssize_t element_size = element[-1].element_size;
            if( length >= 0 ) {
                ...
                array[i] = bcalloc( size, element_size, -1, nref,
                                    EXCEPTION );
            }
...
int TRY( INSTRUCTION_FN_ARGS )
{
    ...
    interpret_exception_t *rg_store = bcalloc
        ( sizeof(interpret_exception_t), sizeof(interpret_exception_t),
          -1, INTERPRET_EXCEPTION_PTRS, EXCEPTION );
...
int ALLOCSTDIO( INSTRUCTION_FN_ARGS )
{
	...
	case 0:
	    ...
	    current_file->filename = bcstrdup( "-", EXCEPTION );
	    break;
	case 1:
	...
	case 2:
	    ...
	    current_file->filename = bcstrdup( "<stderr>", EXCEPTION );
	    break;
	default:
	    assert( 0 );
	}
...
int FDFILE( INSTRUCTION_FN_ARGS )
{
    ...
    case 0:
        file->fp = stdin;
        file->filename = bcstrdup( "-", EXCEPTION );
        break;
    case 1:
        file->fp = stdout;
        file->filename = bcstrdup( "-", EXCEPTION );
    ...
    case 2:
        file->fp = stderr;
        file->filename = bcstrdup( "<stderr>", EXCEPTION );
        break;
    default:
        assert( 0 );
    }
...
int STRLEN( INSTRUCTION_FN_ARGS )
{
    ...
    ssize_t size = block ? block[-1].length : 0;
                     ...
                     cexception_t *ex )
{
    ...
    value = bcalloc_array( size + 1, size + 1, 0, EXCEPTION );
    if( !value ) return 0;
