...
int MKARRAY( INSTRUCTION_FN_ARGS )
{
    ...
    ptr = bcalloc_array( sizeof(stackcell_t), nele, nref, EXCEPTION );
...
int PMKARRAY( INSTRUCTION_FN_ARGS )
{
    ...
    ptr = bcalloc_array( sizeof(stackcell_t), nele, 1, EXCEPTION );
...
/*
 * DEEPCLONE Clone object on the top of the stack, up to the 'nlevels'
 * levels deep.
 *
 * bytecode:
 * DEEPCLONE
...
int ALLOCENV( INSTRUCTION_FN_ARGS )
{
	...
	if( n > 0 ) {
	    env = bcalloc_array( sizeof(stackcell_t), n, 1, EXCEPTION );
...
int ALLOCSTDIO( INSTRUCTION_FN_ARGS )
{
    ...
    files = bcalloc_array( sizeof(stackcell_t), n, 1, EXCEPTION );
    BC_CHECK_PTR( files );
...
int NEXT( INSTRUCTION_FN_ARGS )
{
    ...
    if( node_ref ) {
        if( current_value_offset != value_offset ) {
            ...
            next_ref = ((stackcell_t*)node_ref + next_offset)->PTR;
            if( !next_ref ) {
