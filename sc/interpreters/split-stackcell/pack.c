                         ...
                         cexception_t *ex )
{
    ...
    char *descr_ptr;
    ssize_t size, length, count, start;
    ssize_t element_size = element_nref > 0 ? REF_SIZE : sizeof(stackunion_t);
    char *count_str;
    char typechar;
    ssize_t i;
    ...
    array_ptr = bcalloc_array( REF_SIZE, count, element_nref, ex );
...
{
    if( level == 0 ) {
	if( !unpack_array_values( blob, array, element_nref, description,
                                  offset, unpack, ex )) {
	    return NULL;
	}
    } else {
	...
	for( i = 0; i < layer_len; i++ ) {
	    ...
	    if( !unpack_array_layer( blob, &layer[i], element_nref,
                                     description, offset, level - 1,
                                     unpack, ex )) {
                return NULL;
            }
	}
