...
for( var argnr = 1; argnr <= last(argv); argnr++ ) {
    try {
        ...
        var perm = [ iheader[12]-1, iheader[11]-1, iheader[10]-1 ];
        ...
        var xsize = xintervals; // crs_size[mapX];
        var ysize = yintervals; // crs_size[mapY];
        var zsize = zintervals; // crs_size[mapZ];
        ...
        for var i = 0 to last(raster) {
            for var j = 0 to last(raster[i]) {
                for var k = 0 to last(raster[i][j]) {
                    if( <double>raster[i][j][k] > average ) {
                        atom_number ++;
