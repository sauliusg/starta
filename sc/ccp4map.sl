...
//$Revision: 48 $
//$URL: svn+ssh://saulius-grazulis.lt/home/saulius/svn-repositories/sgem-xray/trunk/bin/ccp4map.sl $
//------------------------------------------------------------------------------
//*
// Read a CCP4 map.
//**
        ...
        for var i = 0 to last(raster) {
            for var j = 0 to last(raster[i]) {
                for var k = 0 to last(raster[i][j]) {
                   sum += raster[i][j][k]@double
                }
            }
        }
        var average = sum / (iheader[0] * iheader[1] * iheader[2])@double;
            ...
            for var j = 0 to last(raster[i]) {
                for var k = 0 to last(raster[i][j]) {
                    if( raster[i][j][k] > map_sigma ) {
                        ...
                        var fx, fy, fz : float;
                        fx = (i+xstart)@float/xintervals@float;
                        fy = (j+ystart)@float/yintervals@float;
                        fz = (k+zstart)@float/zintervals@float;
